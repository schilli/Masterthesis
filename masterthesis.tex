...
In particular, it uses the general amber force field (GAFF) \cite{GAFF} to generate a topologies for small organic molecules that can directly be used with GROMACS in combination with one of the amber force fields.
GAFF is an empirical approach to force field parameterization for arbitraty molcecules.
It inherits the Van-der-Waals paramters from the \texttt{amber99} force fields directly.
Paramters for the bonded interactions are fittet to \textit{ab initio} calculations and equilibrium bond length and angles have also been estimated by real experiments.
A crucial point in topology generation with GAFF is the assignment of partical charges.
Ideally, this should be done based on elaborous quantum chemical calculations.
...
This model tries to generate chartges that emulate the electrostatic potential of \textit{ab initio} Hartree Fock calculations with the HF/6-31G* basis set.
...
The electrostatic potential decays relaively slowly with increasing distance (equation \ref{eq:CoulombPot}).
As a result, the computation of electrostatic interactions converges slowly and cannot simply be cut off at a specified distance without introducing significatn artifacts \cite{FrenkelSmit}.
...
All approaches to tackle long range electorstatics efficently rely either on multipole expansions of or on summations in reciprocal space that converge much faster than the corresponding sums in direct space.
...
The short range part is evaluated directly.
The long range part is then computed in reciprocal space by Fourier transforming the charge distribution, performing the evaluation of the fast converging reciprocal electrostatic interaction function and backtransforming the result to direct space.
...
A method that scales a lot better, at the order of $\mathcal{O}(n log(n))$, is the Particle-Mesh-Ewald method (PME) \cite{PME}.
...
The grid can then be transformed to reciprocal space utilizing the fast Fourier transform algorithm (FFT), which scales itself as $\mathcal{O}(n log(n))$.
After evaluation of the interaction in reciprocal space has happened, the inverse FFT transforms the result back to direct space.
...
These imaginary distributions of states at thermodynamic equilibrium are summarized in the concept of ensembles \cite{Atkins}. 
...
The thermodynamic ensemble that is produced by a molecular dynamics simulation is controled by the macroscopic properties that are kept constant during the simulation
...
This would yield distributions from the microcanonical, or $NVE$ ensemble, which does not correspond to any situation that is easy to create in a laboratory.
Two other ensembles result in distributions that are much more directly comparable to real world experiment.
The canonical, or $NVT$ ensemble, requires a proper control of temperature and the and reflects experimental conditions of constant volume and constant temperature.
The grand canonical ensemble, or $NPT$ ensemple, needs an additional mechanism to control the pressure.
$NPT$ is the desired ensemble for most biomolecular simulations, as it corresponds directly to \textit{in vivo} situations, where the pressure and temperature are controlled by the sourroundings and in general thought to remain constant on chemically relevant time scales.

Several algorithms exist for controlling the pressure durin MD simulations.
...
The Nose-Hoover thermostat (\cite{Nose}, \cite{Hoover}) extends the system Hamiltonian by coupling it to a thermal reservoir with a friction term $\xi$.
...
The Nose-Hoover thermostat can be extended to couple the heat reservoir to another bath and so on to generate a chain of baths.
This Nose-Hoover chain is guaranteed to be ergodic in the limit of infinite chain length.
The system temperature under control of a Nose-Hoover chain will oscillate around a reference value and it is therefore not suited for equilibration if the system starts out at a totally different temperatrure.
...
This is mainly due to the complex interactions between watermolecules that form complicated networks, even in the liquid phase.
A wide range of models have been invented, some of which get close to reality in the predcition of water properties like density, dipole moment, dielectic constant and the self diffusion, but none of them can be regarded as perfect as each has its own shortcomings.
...
This latter was introduced to improve on the dipole moment of real water.
...
All runs were performed with the CHARMM22 all-atom protein force field \cite{CHARMM22} was used together with the implicit solvent model FACTS \cite{FACTS}.
For FACTS, the solute dielectric constant was set to $\epsilon = 1.0$ and the nonpolar surface tension coefficient to $\gamma = 0.015 kcal/(mol A^2)$.
Only dihedral angle Monte Carlo moves have been performed for specified $\phi$ and $\psi$ backbone dihedral angles in the linker part between the two proteins.
...
Apart from that, three runs with 1000 Basin Hopping steps and different vlaues for the sloppy convergence criterium during energy minimization were performed: 0.05, 0.1 and 0.5 kcal/mol rms force.
In addition 40 energy basin structure from previous research have been acquired.
Of each Basin Hopping run the 10 structures with minimal energies were kepts for subsequent analysis.
A total of 120 Structures were obtained in this way.
...
To arrive at a quantitative description of the CitAP binding pocket gemoetry, two distances have been defined (Figure \ref{fig:CitA_pocket}).
Leucine 102 occupies the outermost point of the minor loop.
Its backbone nitrogen atom forms a hydrogen bond with the citrate molecule when the pocket is in its closed conformation.
Glutamic acid 74 is located opposite to Leucine 102 in the binding pocket and Tyrosine 56 sits at the bottom of the acitve site.
From a visual inspection of the open and closed conformations it is apparent that the distances between the backbone nitrogen of Leucine 102 and the \ce{C_{$\alpha$}} atoms of Glutamic acid 74 and Tyrosine 56 give a good geometrical description, as they are minimal in the closed and maximal in the open conformation.
The crystal structure distances of the LEU102:N to TYR56:Ca and LEU102:N to GLU74:Ca are 9.3 and 9.9 \r{A}ngstrom respectively.

The results of 100 ns molecular dynamics simulations of CitA with and without a bound citrate molecule are shown in Figure \ref{fig:CitA_opening_distances}.
The starting structure was for both simulations the citrate bound crystal structure except for the citrate molecule.
While the citrate free pocket opens up within nanoseconds and stays in the open conformation for the entire course of the simulation, the citrate interactions keep the minor loop tightly closed, covering citrate in the binding pocket.
For the first half of the simulation, the LEU102:N to GLU74:Ca distance gets even smaller than the crystal structure reference distance.
...
After approximately 50 ns, the two distances cross in Figure \ref{fig:CitA_opening_distances} meaning that the LEU102:N to TYR56:Ca distance now gets slightly longer than before and the LEU102:N to GLU74:Ca slightly shorter.
