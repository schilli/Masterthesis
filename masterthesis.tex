...
A naive approach would be to use a simple euler integration scheme of a Taylor expansion of the particle position truncated after the second order term:
...
Although this algorithm might seem reasonably fast at first, it turns out to be a particularly bad choice as it is irreversible and suffers from an inherent energy shift \cite{FrenkelSmit}.
Several different integration schemes have been introduced over the years.
It is easiest to start the discussion with the Verlet algorithm.
This algorithm is based on Taylor expansions of the particle position as a function of time around points shifted $+\Delta t$  and $-\Delta t$ in time:
...
Summing equations \ref{eq:Verlet1} and \ref{eq:Verlet2}, solving for $\vfun{r}{t+\Delta t}$ and truncating all terms of order $\mathcal{O}(\Delta t^4)$ or higer yields:

\begin{equation}
    \vfun{r}{t+\Delta t} \approx 2\vfun{r}{t} - \vfun{r}{t-\Delta t} + \frac{\vfun{f}{t}}{m} \Delta t^2 
    \label{eq:VerletPos}
\end{equation}
...
Now one order of accuracy for the velocity computation has been gained at the cost of one order of accuracy for the position update compared to the Verlet integration.
...
It is however possible to get rid of this additional problem by casting the Verlet algorithm in a different form, the velocity Verlet algorithm.
The equation for the position update is the same as in the Euler integrator.
...
In case this accuracy is high enough, the velocity Verlet algorithm is the best choice among the presented algorithms, the only problem being that it its implementation in the latest version of the molecular dynamics package GROMACS does not support all temperature and pressure control mechanisms.
